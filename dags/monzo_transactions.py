...
@dag(
    ...
    schedule_interval="0 7,10,13,16,19,22,23 * * *",
...
def monzo_transactions():
    ...
    def get_monzo_transactions(monzo_auth_obj):
        ...
        trn = FetchTransactions(monzo_auth=monzo_auth_obj)

        all_transactions = []

        # main accounts
        main_accounts = Account.fetch(auth=monzo_auth_obj)

        for acc in main_accounts:
            print(acc.account_id, acc.description)
            main_transactions = trn.fetch_transactions(account_id=acc.account_id)
        ...
        for live_pot in pots_lst_filter:
            ...
            pot_transactions = trn.fetch_transactions(account_id=pot_account_id)
    ...
    upload_monzo_transactions(pull_transactions)
